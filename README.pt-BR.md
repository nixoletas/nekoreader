...
<p align="center">
  ...
  <img alt="Inglês e português" src="https://img.shields.io/badge/idiomas-en%20%C2%B7%20pt--BR-b78a34?style=flat-square&labelColor=211c16">
</p>
...
| Entrar com o Google — o único jeito | `/login` |
...
| Interface em inglês e português, com seletor à mão | toda tela |

### Idiomas

Inglês (padrão e fallback) e português do Brasil. O idioma sai, nesta ordem, do
cookie `neko_lang` (a escolha da pessoa), do `Accept-Language` do navegador (o
palpite) e do padrão — quem pede qualquer outro idioma cai no inglês.
...
navegador baixa **um** idioma, não os dois.
...
literais alargados, então uma chave nova quebra o `tsc` no `pt-BR.ts` até ser
preenchida. Tradução faltando é erro de compilação, não uma tela pela metade.

Foram seis idiomas até aqui. Espanhol, francês, alemão e italiano saíram porque
tradução sem quem revise envelhece errado — e frase errada na tela é pior que
frase em inglês. A estrutura continua a mesma: acrescentar um idioma é criar o
arquivo e pôr o código em `LOCALES`.

Duas regras que valem a pena lembrar ao mexer:

- **Frase inteira no dicionário, nunca pedaço.** "na página" e "no capítulo" são
  entradas separadas porque em português a preposição muda com o gênero; montar
  `"n" + artigo + nome` só funciona numa língua.
- **Contagem usa `{ one, other }`** e passa pelo `plural()`, que consulta o
  `Intl.PluralRules` do idioma, em vez de comparar com 1 na mão.

O OCR acompanha: `IDIOMAS_OCR` mapeia cada idioma pro dicionário do tesseract
(`eng`, `por+eng`), sempre com inglês junto, porque livro técnico em qualquer
língua vem cheio de termo e nome próprio em inglês.
...
2. **SQL Editor → New query** → cole o conteúdo de
   ...
   o que faltava.
3. **Authentication → Providers → Email**: **desligue**. O app não oferece mais,
   mas enquanto o provider estiver ligado a API ainda aceita cadastro por ele.
4. **Authentication → URL Configuration** (obrigatório pro retorno do Google
   funcionar):
...
### Entrar com o Google (obrigatório)

O Google é o único jeito de entrar — não existe formulário de e-mail e senha. É
uma troca deliberada: o que pesava não era o formulário, era a cauda dele
(confirmar o e-mail, reenviar a confirmação, esquecer a senha, receber o link,
link vencido), meia dúzia de telas e um serviço de e-mail de verdade pra
sustentar antes de alguém ler uma página. O preço está dito na própria tela de
entrar: sem conta Google, não entra.

Configure nesta ordem, porque a tela de consentimento pede a URL da política de
privacidade:

1. **Google Cloud Console → OAuth consent screen**: nome do app, e-mail de
   suporte e o endereço da política (`https://seu-app.vercel.app/privacy`).
2. **Credentials → OAuth Client ID (Web)**: URI de redirecionamento autorizada
   `https://<seu-projeto>.supabase.co/auth/v1/callback`.
3. **Supabase → Auth → Providers → Google**: cole o Client ID e o Secret.

Duas coisas que vale saber antes. A tela do Google vai dizer "continuar para
`<seu-projeto>.supabase.co`", e não o nome do app — o redirecionamento do OAuth
é do Supabase, e só um domínio próprio lá (add-on pago) muda isso. E os escopos
básicos (e-mail e perfil) não precisam de verificação do Google nem têm teto de
100 usuários; isso só vale pra escopo sensível.

Enquanto o provider não estiver ligado, o botão responde com um recado em gente
em vez do "Unsupported provider" cru do Supabase.

Quem já tinha conta com o mesmo e-mail, confirmado, continua com ela: o Supabase
gruda a identidade do Google no usuário que existe em vez de criar um segundo.
...
O `tsc` também é teste aqui: `npm run typecheck` é o que cobra a tradução
portuguesa de toda chave nova.
...
src/
  ...
  lib/
    ...
    legal.ts                   a política de privacidade nos dois idiomas
  ...
  app/
    ...
    login/                     entrar com o Google (o único provedor)
    auth/callback              troca por sessão o `code` que o Google devolve
    auth/signout               sair
